...
class ProductService implements ProductServiceInterface {
  ...
  Future<Product?> getProductDetails(
      {required int id, required bool isCampaign}) async {
    return await productRepositoryInterface.get(id.toString(),
  ...
  List<List<int?>> initializeVariationsStock(List<Variation>? variations) {
    ...
    if (variations != null) {
      for (int index = 0; index < variations.length; index++) {
        variationsStock.add([]);
        for (int i = 0; i < variations[index].variationValues!.length; i++) {
          variationsStock[index]
              .add(variations[index].variationValues![i].currentStock);
        }
      }
    }
  ...
  @override
  int setAddonQuantity(
      int addOnQty, bool isIncrement, String? stockType, int? addonStock) {
    int qty = addOnQty;
    if (isIncrement) {
  ...
  int setQuantity(
    ...
    int qty = quantity;
    if (isIncrement) {
      qty = _quantityLimitCheck(selectedVariations, variationsStock,
          cartQuantityLimit, quantity, stockType, itemStock, isCampaign);
    } else {
      qty = qty - 1;
    }
    return qty;
  ...
  int _quantityLimitCheck(
    ...
    int qty = quantity;
    int? minimumStock;
    if (_haveSelectedVariation(selectedVariations) &&
        stockType != 'unlimited' &&
        !isCampaign) {
      minimumStock =
    ...
    if (!isMultiSelect) {
      for (int j = 0; j < resultVariations[index].length; j++) {
        if (variations![index].variationValues![i].stockType != 'unlimited' &&
            variations[index].variationValues![i].currentStock != null &&
            variations[index].variationValues![i].currentStock! <= 0) {
          break;
        }

        if (variations[index].required!) {
          if (variations[index].variationValues![j].stockType != null) {
            if (variations[index].variationValues![j].stockType ==
                    'unlimited' ||
      ...
      if (!resultVariations[index][i]! &&
        ...
        if (variations![index].variationValues![i].stockType != null) {
          ...
          } else if (variations[index].variationValues![i].stockType !=
                  'unlimited' &&
              variations[index].variationValues![i].currentStock! > 0) {
            resultVariations[index][i] = !resultVariations[index][i]!;
          } else {
