import 'package:zenan/features/checkout/widgets/custom_date_picker.dart';
...
class SubscriptionPauseDialog extends StatefulWidget {
  ...
  const SubscriptionPauseDialog(
      {super.key, required this.subscriptionID, required this.isPause});

  @override
  State<SubscriptionPauseDialog> createState() =>
      _SubscriptionPauseDialogState();
}
...
class _SubscriptionPauseDialogState extends State<SubscriptionPauseDialog> {
  ...
  void initState() {
    ...
    if (Get.find<OrderController>().orderCancelReasons != null &&
        Get.find<OrderController>().orderCancelReasons!.isNotEmpty) {
      ...
      for (int index = 0; index < _reasons.length; index++) {
        _cancelReasons.add(DropdownMenuItem<int>(
          value: index,
          child: Padding(
            ...
            child: SizedBox(
                height: 30,
                child: Center(
                    child: Text(_reasons[index].reason!,
                ...
                child: GetBuilder<OrderController>(builder: (orderController) {
                  return Column(mainAxisSize: MainAxisSize.min, children: [
                    ...
                    Padding(
                      ...
                      child: Text(
                        widget.isPause
                            ? 'are_you_sure_to_pause_subscription'.tr
                            : 'are_you_sure_to_cancel_subscription'.tr,
                        style: robotoMedium.copyWith(
                            fontSize: Dimensions.fontSizeLarge),
                        textAlign: TextAlign.center,
                      ),
                    ...
                    GetBuilder<OrderController>(builder: (orderController) {
                      return !orderController.subscriveLoading
                          ? Row(children: [
                              Expanded(
                                  child: TextButton(
                                onPressed: () => Get.back(),
                              ...
                              Expanded(
                                ...
                                onPressed: () {
                                  if (widget.isPause && _range == null) {
                                    showCustomSnackBar(
                                        'choose_subscription_pause_date'.tr);
                                  } else if (!widget.isPause &&
                                      orderController.cancellationIndex == 0) {
                                    showCustomSnackBar(
                                        'please_select_cancellation_reason_first'
                                            .tr);
                                  } else {
                                    orderController.updateSubscriptionStatus(
                                      ...
                                      _range?.start,
                                      _range?.end,
                                      widget.isPause ? 'paused' : 'canceled',
                                      _noteController.text.trim(),
                                      _reasons[orderController
                                              .cancellationIndex!]
                                          .reason,
                                    );
                                  }
