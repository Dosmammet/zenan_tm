...
import 'package:zenan/features/checkout/domain/models/date_month_body_model.dart';
import 'package:zenan/features/checkout/widgets/slot_widget.dart';
...
import 'package:zenan/helper/date_converter.dart';
...
class _TimeSlotBottomSheetState extends State<TimeSlotBottomSheet> {
  ...
  void initState() {
    super.initState();
    _instanceOrder = (Get.find<SplashController>().configModel!.instantOrder! &&
        widget.restaurant.instantOrder!);
  ...
  Future<void> initializeTimeSlots(bool willDelay) async {
    ...
    if (selectedDateSlotIndex == 0) {
      Get.find<CheckoutController>()
          .updateDateSlot(DateTime.now(), _instanceOrder);
      selectedTimeSlot = _createTime(selectedTimeSlotIndex);
    } else if (selectedDateSlotIndex == 1) {
      Get.find<CheckoutController>()
          .updateDateSlot(DateTime.now().add(const Duration(days: 1)), true);
      selectedTimeSlot = _createTime(selectedTimeSlotIndex);
    } else if (selectedDateSlotIndex == 2) {
      Get.find<CheckoutController>()
          .updateDateSlot(selectCustomDate ?? DateTime.now(), _instanceOrder);
      Get.find<CheckoutController>().setDateCloseRestaurant(
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      width: context.width,
      constraints:
          BoxConstraints(maxHeight: context.height * 0.8, minHeight: 0),
                    ...
                    child: Row(children: [
                      Expanded(
                        child: tabView(
                            context: context,
                            title: 'today'.tr,
                            isSelected: selectedDateSlotIndex == 0,
                            onTap: () {
                              setState(() {
                      ...
                      Expanded(
                        child: tabView(
                            context: context,
                            title: 'tomorrow'.tr,
                            isSelected: selectedDateSlotIndex == 1,
                            onTap: () {
                              setState(() {
                      ...
                      (isRestaurantSelfDeliveryOn
                              ? widget.restaurant.customerDateOrderStatus!
                              : Get.find<SplashController>()
                                  .configModel!
                                  .customerDateOrderStatus!)
                          ? Expanded(
                              child: tabView(
                                  context: context,
                                  title: 'custom_date'.tr,
                                  isSelected: selectedDateSlotIndex == 2,
                                  onTap: () {
                                    setState(() {
                      ...
                      child: Column(
                          ...
                          children: [
                            selectedDateSlotIndex == 2
                                ? Column(children: [
                                    Center(
                                        child: Text('set_date_and_time'.tr,
                                    ...
                                    SfDateRangePicker(
                                        ...
                                        onSelectionChanged:
                                            (DateRangePickerSelectionChangedArgs
                                                args) {
                                          DateTime selectedDate = DateConverter
                              ...
                              child: Row(children: [
                                ...
                                Expanded(
                                  child: CustomButtonWidget(
                                    ...
                                    onPressed: () {
                                      checkoutController.updateDateSlotIndex(
                                          selectedDateSlotIndex);
                                      ...
                                      checkoutController.setCustomDate(
                                          selectCustomDate,
                                          _instanceOrder &&
                                              DateTime(
                                                      DateTime.now().year,
  ...
  String _createCustomTime(int index) {
    String time = (index == 0 &&
            selectedDateSlotIndex == 2 &&
            Get.find<RestaurantController>().isRestaurantOpenNow(
                Get.find<CheckoutController>().restaurant!.active!,
                Get.find<CheckoutController>().restaurant!.schedules) &&
            DateTime(DateTime.now().year, DateTime.now().month,
                    DateTime.now().day)
                .isAtSameMomentAs(selectCustomDate ??
                    DateTime(DateTime.now().year, DateTime.now().month,
                        DateTime.now().day))
        ? _instanceOrder
            ? 'now'.tr
            : 'not_available'.tr
        : '${DateConverter.dateToTimeOnly(Get.find<CheckoutController>().timeSlots![index].startTime!)} '
