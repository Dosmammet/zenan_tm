...
class CouponRepository implements CouponRepositoryInterface {
  ...
  Future<List<CouponModel>?> getList(
      ...
      int? customerId,
      int? restaurantId,
      bool fromRestaurant = false}) async {
    if (fromRestaurant) {
  ...
  Future<Response> applyCoupon(String couponCode, int? restaurantID) async {
    return await apiClient.getData(
        '${AppConstants.couponApplyUri}$couponCode&restaurant_id=$restaurantID',
        handleError: true,
        showToaster: true);
  }

  Future<List<CouponModel>?> _getCouponList(
      int? customerId, int? restaurantId) async {
    List<CouponModel>? couponList;
    Response response = await apiClient.getData(
        '${AppConstants.couponUri}?${restaurantId != null ? 'restaurant_id' : 'customer_id'}=${restaurantId ?? customerId}');
  ...
  Future<List<CouponModel>?> _getRestaurantCouponList(int restaurantId) async {
    List<CouponModel>? couponList;
    Response response = await apiClient.getData(
        '${AppConstants.restaurantWiseCouponUri}?restaurant_id=$restaurantId');
    if (response.statusCode == 200) {
      couponList = [];
      response.body.forEach((category) {
        couponList!.add(CouponModel.fromJson(category));
