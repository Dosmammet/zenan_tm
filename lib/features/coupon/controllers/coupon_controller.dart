...
class CouponController extends GetxController implements GetxService {
  ...
  Future<void> getRestaurantCouponList({required int restaurantId}) async {
    _couponList = await couponServiceInterface.getRestaurantCouponList(
        restaurantId: restaurantId);
    // _toolTipController = couponServiceInterface.generateToolTipControllerList(_couponList);
    update();
  }

  Future<double?> applyCoupon(String coupon, double order,
      double deliveryCharge, double charge, double total, int? restaurantID,
      {bool hideBottomSheet = false}) async {
    _isLoading = true;
    _discount = 0;
    update();
    Response response =
        await couponServiceInterface.applyCoupon(coupon, restaurantID);
    if (response.statusCode == 200) {
      ...
      if (Get.find<CheckoutController>().isPartialPay ||
          Get.find<CheckoutController>().paymentMethodIndex == 1) {
